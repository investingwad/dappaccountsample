...
void required_key(name vaccount){ \
  vkeys_t vkeys_table(_self, _self.value, 1024, 64, VACCOUNTS_SHARD_PINNING, false, VACCOUNTS_DELAYED_CLEANUP); \
    ...
    auto it = std::find (existing->pubvec.begin(), existing->pubvec.end(), _pubkey); \
    eosio::check(it !=  existing->pubvec.end(), "wrong public key"); \
} \
...
void unpack_exec_action(eosio::action res){ \
    switch(res.name.value){ \
      case name("regaccount").value: \
        ...
        break; \
      case name("updateacckey").value: \
        updateacckey(res.data_as<regaccount_action>()); \
        break; \
      default: \
...
TABLE vkey { \
  ...
  name vaccount; \
  uint64_t nonce; \
  std::vector<eosio::public_key> pubvec; \
...
[[eosio::action]] void updateacckey(regaccount_action payload) { \
    print("adding key for:");\
    print(payload.vaccount);\
    print("\n");\
   addKey(payload.vaccount, get_current_public_key()); \
} \
...
void require_vaccount(name vaccount){ \
    ...
    required_key(vaccount); \
} \
void setKey(name vaccount, eosio::public_key pubkey){ \
    vkeys_t vkeys_table(_self, _self.value, 1024, 64, VACCOUNTS_SHARD_PINNING, false, VACCOUNTS_DELAYED_CLEANUP); \
    auto existing = vkeys_table.find(vaccount.value); \
    std::vector<eosio::public_key> temp;\
    temp.push_back(pubkey);\
    eosio::check(existing == vkeys_table.end(),"vaccount " + vaccount.to_string() + " already registered. Call updateacckey to add keys" ); \
...
void addKey(name vaccount, eosio::public_key pubkey){ \
    vkeys_t vkeys_table(_self, _self.value, 1024, 64, VACCOUNTS_SHARD_PINNING, false, VACCOUNTS_DELAYED_CLEANUP); \
    auto existing = vkeys_table.find(vaccount.value); \
    eosio::check(existing != vkeys_table.end(),"vaccount not registered"); \
    auto it = std::find (existing->pubvec.begin(), existing->pubvec.end(), pubkey); \
    eosio::check(it ==  existing->pubvec.end(), "Provided public key already registered for this account"); \
    auto vec = existing->pubvec; vec.push_back(pubkey);\
